...
extends Button_3D {
    ...
    void unroll() {
        if (!this.Unrolled && !Main.MainRef.RolledOut) {
            ...
            while (n < this.KeywordCount) {
                this.Items[n].show(this.TopLeftX + 10, this.TopLeftY + 47 + n * 32);
                this.Cost[n].show(this.TopLeftX + 220, this.TopLeftY + 47 + n * 32);
                ++n;
            }
            this.Unrolled = true;
        }
    ...
    public Button_3DWeapon(int n, int n2, int n3, String[] stringArray, int[] nArray, int[] nArray2, String string, int n4) {
        this.SelectedItem = n4;
        this.Keyword = string;
        this.TextMessages = stringArray;
        this.CostMessages = nArray;
        this.KeywordCount = n3;
        this.Enabled = nArray2;
        this.Items = new Message_3D[this.KeywordCount];
        this.Cost = new Message_3D[this.KeywordCount];
        this.X = n;
        this.Y = n2;
        this.TopLeftX = n - 126;
        this.TopLeftY = n2 - 14;
        ...
        this.Model.setSurfaceShader(Main.MainRef.GlobalMedia.GiltButtonsDrop.Shader);
        ...
        this.RolloutModel.setSurfaceShader(Main.MainRef.GlobalMedia.GiltRollout.Shader);
        ...
        this.Rollout.setOption(0, 103);
        this.RollOverModel = Main.MainRef.Wt.createPlane(0.694944f * Main.MainRef.camera.FOVFactor, 0.087552f * Main.MainRef.camera.FOVFactor, false, 0.0f, 0.0f, 1);
        this.RollOverModel.setSurfaceShader(Main.MainRef.GlobalMedia.GiltButtons.Shader);
        this.RollOverModel.setMaterial(255, 255, 255, 0.0f, 255, 255, 255);
        this.RollOverModel.setTextureRect("front", 0.0f, 0.5f, 1.0f, 1.0f);
        ...
        this.RollOver.setOption(0, 104);
        int n5 = 0;
        while (n5 < this.KeywordCount) {
            if (this.Enabled[n5] == 1) {
                this.Items[n5] = new Message_3D(this.TextMessages[n5], 0, 1.0f, 95);
                this.Cost[n5] = new Message_3D("" + nArray[n5], 2, 1.0f, 95);
            } else if (this.Enabled[n5] == 2) {
                this.Items[n5] = new Message_3D(this.TextMessages[n5], 0, 1.0f, 95, 1);
                this.Cost[n5] = new Message_3D("" + nArray[n5], 2, 1.0f, 95, 1);
            } else {
                this.Items[n5] = new Message_3D(this.TextMessages[n5], 0, 1.0f, 95, 2);
                this.Cost[n5] = new Message_3D("" + nArray[n5], 2, 1.0f, 95, 2);
            }
            ++n5;
        }
        if (this.Enabled[this.SelectedItem] == 1) {
            this.Title = new Message_3D(this.TextMessages[this.SelectedItem], 0, 1.0f, 92);
            this.TitleCost = new Message_3D("" + nArray[this.SelectedItem], 2, 1.0f, 92);
            return;
        }
        if (this.Enabled[this.SelectedItem] == 2) {
            this.Title = new Message_3D(this.TextMessages[this.SelectedItem], 0, 1.0f, 92, 1);
            this.TitleCost = new Message_3D("" + nArray[this.SelectedItem], 2, 1.0f, 92, 1);
            return;
        }
        this.Title = new Message_3D(this.TextMessages[this.SelectedItem], 0, 1.0f, 92, 2);
        this.TitleCost = new Message_3D("" + nArray[this.SelectedItem], 2, 1.0f, 92, 2);
    }
    ...
    public void show() {
        if (!this.Visible) {
            ...
            this.TitleCost.show(this.TopLeftX + 220, this.TopLeftY + 16);
    ...
    void setSelection(int n) {
        ...
        this.Title.destroy();
        this.TitleCost.destroy();
        this.Title = this.Enabled[this.SelectedItem] == 1 ? new Message_3D(this.TextMessages[this.SelectedItem], 0, 1.0f, 92) : (this.Enabled[this.SelectedItem] == 2 ? new Message_3D(this.TextMessages[this.SelectedItem], 0, 1.0f, 92, 1) : new Message_3D(this.TextMessages[this.SelectedItem], 0, 1.0f, 92, 2));
        this.Title.show(this.TopLeftX + 10, this.TopLeftY + 16);
        this.TitleCost = this.Enabled[this.SelectedItem] == 1 ? new Message_3D("" + this.CostMessages[this.SelectedItem], 2, 1.0f, 92) : (this.Enabled[this.SelectedItem] == 2 ? new Message_3D("" + this.CostMessages[this.SelectedItem], 2, 1.0f, 92, 1) : new Message_3D("" + this.CostMessages[this.SelectedItem], 2, 1.0f, 92, 2));
        this.TitleCost.show(this.TopLeftX + 220, this.TopLeftY + 16);
    }
}
