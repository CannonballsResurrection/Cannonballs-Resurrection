...
public class Particle_Object_Firefly
...
WTConstants {
    ...
    public Particle_Object_Firefly(float f, float f2, float f3) {
        ...
        this.Z = f3;
        this.show();
        this.Scale = 2.0f + Main.MainRef.random.nextFloat();
        ...
        this.HidTimer = Main.MainRef.random.nextFloat() * 5.0f;
        this.X += (Main.MainRef.random.nextFloat() - 0.5f) * 20.0f;
        this.Y += (Main.MainRef.random.nextFloat() - 0.5f) * 20.0f;
        this.Z += (Main.MainRef.random.nextFloat() - 0.5f) * 20.0f;
        this.Pos.fill(Main.MainRef.random.nextFloat() - 0.5f, Main.MainRef.random.nextFloat() - 0.5f, Main.MainRef.random.nextFloat() - 0.5f);
        this.Pos.normalize();
        this.Pos.multiply(10.0f + Main.MainRef.random.nextFloat() * 20.0f);
    ...
    public void updateTimeSlice(float f) {
        ...
        this.HidTimer -= f;
        if (this.Visible && this.HidTimer <= 0.0f) {
            this.hide();
            this.HidTimer = 1.0f + Main.MainRef.random.nextFloat() * 2.0f;
        } else if (!this.Visible && this.HidTimer <= 0.0f) {
            this.show();
            this.HidTimer = 1.0f + Main.MainRef.random.nextFloat() * 6.0f;
        }
        if (this.Visible) {
            this.Container.setPosition(this.X + this.Pos.X, this.Y + this.Pos.Y * 0.2f, this.Z + this.Pos.Z);
