...
final class BotState {
    ...
    func think(for c: Cannon, game: GameController, dt: Float) {
        ...
        if game.currentPlayerIndex == c.index {
            ...
            // Fidelity: the original AIThink runs every frame and re-acquires a
            // target whenever it has none, dropping any target that has gone
            // inactive (Cannon.java:644-669). The port otherwise solves only once
            // in beginTurn, so a turn that began without a valid target (a
            // transient race: the intended enemy inactive/dying that frame) — or
            // whose target later went inactive — would leave haveSolution == false
            // forever: slew() no-ops (BotAI.swift:222), the fire path is guarded
            // off below, nothing sets waitingTimer, and the turn hangs. Mirror the
            // original's per-frame re-acquisition so the barrel always re-solves
            // toward a live enemy and the turn cannot strand. This runs only when
            // there is no usable solution, so it never disturbs the once-per-shot
            // walking-fire aim (computeSolution advances the solution per shot).
